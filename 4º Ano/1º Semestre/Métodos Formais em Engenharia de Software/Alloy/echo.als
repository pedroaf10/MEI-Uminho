...
fact init {
	all n: Node | no n.rcvd and no n.parent and no n.children
	one initiator
}
...
assert Invariantes {
	// O initiatior nunca tem pai.
 	always (all i: initiator | no i.parent)
	// O pai tem sempre que ser um dos vizinhos.
	always (all n: Node | one n.parent implies n.parent in n.adj)
	// Um nó só pode ser filho do seu pai.
	always (all n: Node | one n.parent implies one children.n)
}
check Invariantes
